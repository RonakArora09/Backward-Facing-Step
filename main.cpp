...
void init_us(double** u, double** u_hat, double** u_face, double* u_in){
    ...
    for(i=0; i<nx+2; i++)
    {
        for(j=0; j<ny+2; j++)
        {
            u[i][j] = u_hat[i][j] = u_face[i][j] = u_in[j];
        }
    }
    for(j=0; j<ny+2; j++)
    {
        u_face[nx+2][j] = u_in[j];
    }
...
int main(){
    ...
    double alpha_uv = 0.7, alpha_p = 0.4;
    ofstream outfile;
    ...
    double* u_in = new double[ny+2];
    double U_max = 0.1*1.5;

    // Input Velocity Profile
    for(j=0; j<ny+2; j++)
    {
        u_in[j] = 4*U_max*(((j-ny_in-0.5)*dy)/height_in)*(1-((j-ny_in-0.5)*dy)/height_in);
    }
